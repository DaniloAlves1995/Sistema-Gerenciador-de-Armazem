...
public class SacaDao {
    ...
    public List<Saca> getLista(String dado, String data, int tipo, String produto) throws SQLException{
        ...
        if(tipo==0){
            ...
            sql="select produto.nome_p, caminhao.nome_ca, sacas.* from sacas, produto, caminhao WHERE sacas.data between '?' and '?' and produto.nome_p LIKE '%?%' and produto.id_p = sacas.id_p and sacas.id_ca=caminhao.id_ca;";
        }else{
            sql="select produto.nome_p, caminhao.nome_ca, sacas.* from produto, caminhao, sacas WHERE sacas.data between '?' and '?' and produto.id_p=sacas.id_p and caminhao.id_ca = sacas.id_ca;";
        }
